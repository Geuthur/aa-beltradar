...
class TestAddSurveyForm(NoSocketsTestCase):
    ...
    def test_raw_data_required(self):
        form = AddSnapshotForm(data={"raw_data": ""})
    ...
    def test_parse_ore_data_valid(self):
        ...
        form = AddSnapshotForm(data={"raw_data": raw_data})
        self.assertTrue(form.is_valid())
        parsed_ores = form.cleaned_data["raw_data"].ore_list
        self.assertEqual(len(parsed_ores), 1)
        self.assertEqual(parsed_ores[0].name, "Mercoxit III-Grade")
        self.assertEqual(parsed_ores[0].units, 1433)
        self.assertEqual(parsed_ores[0].volume_m3, 57320)
        self.assertEqual(parsed_ores[0].price_isk, 0.0)  # TODO Make Price Test
    ...
    def test_parse_ore_data_multiple_rows(self):
        ...
        form = AddSnapshotForm(data={"raw_data": raw_data})
        self.assertTrue(form.is_valid())
        self.assertEqual(len(form.cleaned_data["raw_data"].ore_list), 2)
    ...
    def test_parse_ore_data_price_decimal_comma(self):
        ...
        self.assertTrue(form.is_valid())
        self.assertEqual(
            form.cleaned_data["raw_data"].ore_list[0].price_isk, 0.0
    ...
    def test_parse_ore_data_space_separated_german_format(self):
        ...
        self.assertTrue(form.is_valid())
        parsed_ores = form.cleaned_data["raw_data"].ore_list
        self.assertEqual(parsed_ores[0].name, "Dark Ochre III-Grade")
        self.assertEqual(parsed_ores[0].units, 371)
        self.assertEqual(parsed_ores[0].volume_m3, 2968)
        self.assertEqual(parsed_ores[0].price_isk, 0.0)  # TODO Make Price Test
    ...
    def test_parse_ore_data_ignores_non_data_info_lines(self):
        ...
        self.assertTrue(form.is_valid())
        parsed_ores = form.cleaned_data["raw_data"].ore_list
        self.assertEqual(len(parsed_ores), 2)
        self.assertEqual(parsed_ores[0].name, "Nocxite III-Grade")
        self.assertEqual(parsed_ores[0].units, 45806)
        self.assertEqual(parsed_ores[0].volume_m3, 183224)
        self.assertEqual(parsed_ores[0].price_isk, 0.0)  # TODO Make Price Test
        self.assertEqual(parsed_ores[1].name, "Nocxite IV-Grade")
        self.assertEqual(parsed_ores[1].units, 67345)
        self.assertEqual(parsed_ores[1].volume_m3, 269380)
        self.assertEqual(parsed_ores[1].price_isk, 0.0)  # TODO Make Price Test
...
class TestBeltSurveySessionForm(NoSocketsTestCase):
    ...
    def test_create_survey_session(self):
        form = BeltSessionForm(data={"name": "Test Session"})
        self.assertTrue(form.is_valid())

    def test_create_survey_session_empty_name(self):
        form = BeltSessionForm(data={"name": ""})
        self.assertFalse(form.is_valid())
