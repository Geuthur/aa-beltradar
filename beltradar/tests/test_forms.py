...
class TestAddSurveyForm(NoSocketsTestCase):
    ...
    def test_parse_ore_data_space_separated_german_format(self):
        ...
        form = AddSurveyForm(data={"raw_data": raw_data})
        self.assertTrue(form.is_valid())
        self.assertEqual(form.parsed_items[0].name, "Dark Ochre III-Grade")
        self.assertEqual(form.parsed_items[0].units, 371)
        self.assertEqual(form.parsed_items[0].volume_m3, 2968)
        self.assertEqual(form.parsed_items[0].price_isk, 1540000)
    ...
    def test_parse_ore_data_ignores_non_data_info_lines(self):
        ...
        self.assertTrue(form.is_valid())
        self.assertEqual(len(form.parsed_items), 2)
        self.assertEqual(form.parsed_items[0].name, "Nocxite III-Grade")
        self.assertEqual(form.parsed_items[0].units, 45806)
        self.assertEqual(form.parsed_items[0].volume_m3, 183224)
        self.assertEqual(form.parsed_items[0].price_isk, 52800000)
        self.assertEqual(form.parsed_items[1].name, "Nocxite IV-Grade")
        self.assertEqual(form.parsed_items[1].units, 67345)
        self.assertEqual(form.parsed_items[1].volume_m3, 269380)
        self.assertEqual(form.parsed_items[1].price_isk, 30300000)
